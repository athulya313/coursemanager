...
export const Contact=AsyncError(async(req,res,next)=>{
    ...
    const subject="Contact from Course Manager"

    const text=`Iam ${name} and my Email is ${email}.\n ${message}`
    await sendEmail(to,subject,text)
    res.status(200).json({
        success:true,
        message:'Your message has send'
    })
})

export const courseRequest=AsyncError(async(req,res,next)=>{
...
export const getDashboardStats=AsyncError(async(req,res,next)=>{
    ...
    res.status(200).json({
        ...
        viewsCount,
        subscriptionCount,
        subscriptionPercentage,viewsPercentage,usersPercentage,usersProfit,viewsProfit,subscriptionProfit 
    })
})